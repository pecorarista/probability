...
\begin{definition}
\(X\)を空でない集合とする．
関数\(d\colon X \times X \to \symbb{R}\)が以下の\ref{distance-nonnegativity}--\ref{distance-triangle-inequality}をみたすとき，\(d\)を\(X\)上の\keyword{距離}(metric)または\keyword{距離関数}(distance function)という．
\begin{conditions}
...
\end{conditions}
\ref{distance-nonnegativity}--\ref{distance-triangle-inequality}をまとめて\keyword{距離の公理}という．
...
\end{align*}
は距離の公理をみたす（証明略）．
この\(d\)を\keyword{離散距離}(discrete distance)という．
\end{example}
...
\end{align*}
と定義すると\(d\)は距離の公理をみたす（証明略）．この距離を\keyword{ユークリッド距離}(Euclidean distance)という\figref{euclidean-distance}．
...
\begin{minipage}{0.5\textwidth}
    ...
    \caption{\(\symbb{R}^2\)におけるユークリッド距離．}\figlabel{euclidean-distance}
\end{minipage}
\begin{minipage}{0.5\textwidth}
    \centering
    \input{figures/manhattan-distance.tex}
    \caption{\(\symbb{R}^2\)におけるマンハッタン距離．}\figlabel{manhattan-distance}
\end{minipage}
\end{figure}

\begin{example}
\(\symbb{R}^2\)の任意の\(2\)点\(x = (x_1, x_2), y = (y_1, y_2)\)をとる．
\figref{manhattan-distance}のように，
...
\end{align}
に等しい．
一般に球面とその中心を通る平面が交わってできる円を\keyword{大円}(great circle)という．
また\mathref{great-circle-distance}で定まる距離を\keyword{大円距離}(great circle distance)という（\figref{great-circle-distance}）．
\begin{figure}
    \centering
    \input{figures/great-circle.tex}
    \caption{大円距離．}
    \figlabel{great-circle-distance}
\end{figure}
球面に沿って移動する経路で長さが最短になるのが大円の劣弧になること，
および\mathref{great-circle-distance}が距離の公理をみたすことは\chapterref{geodesic}で示す．
\end{example}
...
\(X\)上の実数値関数\(\lVert \placeholder \rVert\colon x \mapsto \lVert x \rVert\)が次の\ref{norm--nonnegativity}--\ref{norm-triangle-inequality}をみたすとき，これを\(X\)上の\keyword{ノルム}(norm)という．
\begin{conditions}
    \item\label{norm-nonnegativity} 任意の\(x \in X\)で\(\lVert x \rVert \geq 0\)が成り立つ．
        また\(\lVert x \rVert = 0\)となるための必要十分条件は\(x = 0\)であることである．
    \item\label{norm-scalar} 任意の\(\lambda \in \symbb{K}\)と\(x \in X\)について
        \(\lVert \lambda x \rVert = \lvert \lambda \rvert \lVert x \rVert\)
        が成り立つ．
    \item\label{norm-triangle-inequality} （三角不等式，劣加法性）任意の\(x, y \in X\)について\(\lVert x + y \rVert \leq \lVert x \rVert + \lVert y \rVert\)が成り立つ．
\end{conditions}
\ref{norm-nonnegativity}--\ref{norm-triangle-inequality}をまとめて\keyword{ノルムの公理}という．
\(X\)とそのノルムの組\(\tuple{X, \lVert \placeholder \rVert}\)を\keyword{ノルム空間}(normed space)という．\(\lVert \placeholder \rVert\)がどのようなものか明らかな場合，\(\tuple{X, \lVert \placeholder \rVert}\)を単に\(X\)と書く．
\end{definition}
\end{thmbox}
...
\end{align*}
と定義すると\(\lVert \placeholder \rVert\)はノルムの公理をみたす（証明略）．このノルムを\keyword{ユークリッドノルム}(Euclidean norm)という．
...
\begin{example} \(S^1\)を単位円周とする．図形\(T^2 = S^1 \times S^1\)を\keyword{(2次元)トーラス}((2-dimensional) torus)という（\figref{torus}）．
...
\mathref{cover}が成り立ち，かつ\(\symcal{U}\)の元がすべて開集合であるとき，
...
Riemann\footnote{\pronunciation{Riemann}{de}{ˈʀiːman}{リーマン}}積分の定義に用いられる過剰和(upper sum)と不足和(lower sum)はネットの例である．
閉区間\(I\)上で定義される実数値関数\(f\)について，\(I\)の分割\(\Delta = \{x_0, \ldots, x_n\})\)による過剰和と不足和（\figref{partition-of-interval}）を以下で定義する：
\begin{gather}
...
\(\symcal{V} = \{X \setminus A \mid A \in \symcal{A}\}\)とすると，\mathref{fip-de-morgan}と同様，\(X \setminus \bigcup \symcal{V} = \bigcap \symcal{A} = \emptyset\)，すなわち\(X = \bigcup \symcal{V}\)が成り立つ．有限交叉性から\(\{A_1, \ldots, A_n\} \subseteq \symcal{A}\)で
\begin{align*}
