...
\mathref{great-circle-length-integral}の被積分関数を\(F \colon [0, 1] \times \symbb{R}^2 \times \symbb{R}^2 \to \symbb{R}\)とすると
\begin{align*}
    & F\left(
        t,
        \begin{pmatrix}
...
\begin{align*}
    ...
    = 0
\end{align*}
である．\mathref{great-circle-regular}と\(t > 0\)において\(\sin \theta(t) > 0\)であるから，成分ごとに計算して
\begin{numcases}
    \displaystyle
    \frac{%
...
\end{numcases}
を得る．\mathref{great-circle-ode2}から
\begin{align*}
