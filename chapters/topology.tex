...
\(S^2\)の
\(2\)点\(p, q\)をとる．
集合\(G(p, q)\)を以下の\ref{geodesic-example-0-1}--\ref{geodesic-example-c1}をみたす写像\(\gamma \colon [0, 1] \to \symbb{R}^3\)全体の集合とする．
\begin{conditions}
    \item\label{geodesic-example-0-1} \(\gamma(0) = p\)かつ\(\gamma(1) = q\)．
    \item\label{geodesic-example-s2} 任意の\(t \in [0, 1]\)について\(\gamma(t) \in S^2\)．
    \item\label{geodesic-example-c1} \(\gamma\)は連続微分可能である．
        すなわち\(\gamma\)は\([0, 1]\)において微分可能であり，
        導関数\(\dot{\gamma}\)は\([0, 1]\)において連続である．
\end{conditions}
\(p\)と\(q\)について
\begin{align*}
    d(p, q) = \inf \left\{ \int_0^1 \lVert \dot{\gamma}(t) \rVert dt \relmiddle{|} \gamma \in G(p, q) \right\}
\end{align*}
とすると，これは距離の公理をみたす（証明略）．
距離を与える曲線を求めるのは典型的な変分問題である．
\(p\)を北極とする．
以下でこの問題を解く．
\(\gamma\)上の点は余緯度を\(\theta\)，経度を\(\varphi\)として
\begin{align*}
    \gamma(t) =
    \begin{pmatrix}
...
\end{align*}
と表すことができる．
以降スペースの都合上\(\theta(t), \varphi(t)\)を単に\(\theta, \varphi\)と書く．
導関数は
\begin{align*}
    \dot{\gamma}(t) =
    \begin{pmatrix}
