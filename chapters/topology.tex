...
\begin{definition}[（サブネット）] ネット\(g \colon B \to X\)がネット\(f \colon A \to X\)の\keyword{サブネット}(subnet)であるとは，写像\(\varphi \colon B \to A\)が存在して，次の2つをみたすことをいう．
    \begin{conditions}
        ...
        \item\label{subnet-exists-beta} （共終性）任意の\(\alpha \in A\)に対して，ある\(\beta \in B\)が存在して，\(\beta \preceq \beta'\)ならば\(\alpha \preceq \varphi(\beta')\)が成り立つ．
...
サブネットの定義（\definitionref{subnet}）の\ref{subnet-exists-beta}が成り立つことは，任意の\(\alpha \in \Lambda\)について\(\tuple{\alpha, X} \in \symcal{D}\)をとると，
任意の\(\tuple{\beta, U} \succeq \tuple{\alpha, X}\)について\(\alpha \preceq \beta = \varphi(\tuple{\beta, U})\)からわかる．
