...
\begin{thmbox}
\begin{proposition}
位相空間\(\tuple{X, \symcal{O}}\)の点\(x\)の近傍全体の集合を\(\Neighborhood{x}\)とする．
このとき以下が成り立つ．
\begin{conditions}
...
\begin{thmbox}
\begin{definition}[（サブネット）] ネット\(g \colon B \to X\)がネット\(f \colon A \to X\)の\keyword{サブネット}(subnet)であるとは，写像\(\varphi \colon B \to A\)が存在して，次の2つをみたすことをいう．
    \begin{conditions}
        \item\label{subnet-comp} \(g = f \circ \varphi\)．
        \item\label{subnet-exists-beta} （共終性）任意の\(\alpha \in A\)に対して，ある\(\beta \in B\)が存在して，\(\beta' \succeq \beta\)ならば\(\alpha \preceq \varphi(\beta')\)が成り立つ．
    \end{conditions}\definitionlabel{subnet}
\end{definition}
\end{thmbox}

一般に前順序集合\(\tuple{A, \mathord{\preceq}}\)の部分集合\(A' \subseteq A\)が\keyword{共終}(cofinal)であるとは，任意の\(\alpha \in A\)に対してある\(\alpha' \in A'\)が存在して\(\alpha \preceq \alpha'\)となることをいう．また関数\(\varphi \colon B \to A\)が共終であるとは像\(\varphi(B) \subseteq A\)が共終であることをいう．したがって\ref{subnet-exists-beta}は\(\varphi(\{ \beta' \in B \mid \beta' \succeq \beta\})\)が共終であると言い換えることができる．
...
\begin{align*}
    \tuple{\lambda, U} \preceq \tuple{\lambda', U'} \Leftrightarrow \text{\(\lambda \preceq \lambda'\) and \(U \supseteq U'\)}
\end{align*}
と定める．このとき\(\symcal{D}\)は有向集合になることを示す．
反射律と推移律は明らかである．
上界の存在を示す．
...
\(U_3 := U_1 \cap U_2\)は\(U_3 \in \Neighborhood{x}\), \(U_1 \preceq U_3\), \(U_2 \preceq U_3\)をみたす．
...
共終性は任意の\(\alpha \in \Lambda\)について\(\tuple{\alpha, X} \in \symcal{D}\)をとると，
\(\tuple{\beta, U} \succeq \tuple{\alpha, X}\)をみたす任意の\(\tuple{\beta, U}\)について\(\alpha \preceq \beta = \varphi(\beta, U)\)からわかる．

\(f \circ \varphi \to x\)を示す．
\(x\)の任意の開近傍\(U\)に対して，ある\(\lambda_0 \in \Lambda\)で\(x_{\lambda_0} \in U\)となるものが存在する．したがって\(\tuple{\lambda_0, U} \preceq \tuple{\beta, V}\)をみたす任意の\(\tuple{\beta, V} \in \symcal{D}\)について\((f \circ \varphi)(\beta, V) = \varphi(\beta) \in V \subseteq U\)となる．
