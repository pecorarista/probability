...
ある\(\lambda_1 \in \Lambda\)が存在して，任意の\(\lambda \succeq \lambda_1\)で\(x_\lambda \in U\)となる．

また\(\varphi\)の共終性より，ある\(\lambda_2 \in B\)が存在して，
\(\lambda \succeq \lambda_2\)をみたす任意の\(\lambda \in \Lambda\)について\(\varphi(\lambda) \succeq \alpha\)が成り立つ．

\(\Lambda\)が有向集合であるから\(\lambda_0 \succeq \kappa\)が存在する．
