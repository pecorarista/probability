...
点\(x\)の開近傍全体の集合を\(\Neighborhood{x}\)で表す．
...
\end{align*}
と定義する．\(\symcal{D}\)の任意の元\(\tuple{\lambda, U}\)と\(\tuple{\lambda', U}\)について
\begin{align*}
...
\(U := U_1 \cap U_2\)は\(U \in \Neighborhood{x}\), \(U_1 \preceq U\), \(U_2 \preceq N\)をみたす．
\(x\)が\(f\)の集積点なので，この\(U\)に対して，ある\(\lambda \in \Lambda\)が存在して，
\(\lambda \succeq \lambda_1\), \(\lambda \succeq \lambda_2\), \(x_\lambda \in U\)となる．
したがって\(\tuple{\lambda_1, U} \preceq \tuple{\lambda, U}\)かつ\(\tuple{\lambda_2, U_2} \preceq \tuple{\lambda, U}\)となる\(\tuple{\lambda, U}\)の存在がいえた．
