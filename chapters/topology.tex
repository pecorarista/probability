...
地表を球面
\begin{align*}
    S^2(R) = \left\{ \begin{pmatrix} x \\ x \\ x \end{pmatrix} \in \symbb{R}^3 \relmiddle{|} \sqrt{x^2 + y^2 + z^2} = R \right\}
\end{align*}
によってモデル化する．
\(S^2(R)\)上に\(2\)点\(p, q\)をとる．
このとき\(p\)から\(p\)へ球面に沿って移動する経路で長さが最小となるのは，\(p, q\)そして\(S^2\)の中心\(\symup{O}\)によって定まる平面と，球面が交わってできる円の劣弧の長さ
\begin{align}
...
また\eqref{great-circle-distance}で定まる距離を\keyword{大円距離}(great circle distance)という（\figref{great-circle-distance}）．
\begin{figure}
    \centering
...
\begin{numcases}
    ...
    \frac{%
        ...
        \sqrt{(\dot{\theta}(t))^2 + (\dot{\varphi}(t))^2 \sin^2 \theta(t)}
    } \eqlabel{great-circle-ode1} \\[10pt]
    \displaystyle
    0 =
    \frac{d}{dt}
    \frac{%
...
\end{align*}
任意の\(\gamma_1 \in \Omega(p, r)\)と\(\gamma_2 \in \Omega(r, q)\)
\(\gamma = \gamma_1 + \gamma_2\)
\begin{align*}
    d(x, z) < \inf \{L(\gamma) \mid \gamma \in \Omega(x, y)\}
\end{align*}
