...
\begin{conditions}
    \item 任意の\(F \in \symcal{F}\)について\(f\)が最終的に\(F\)に入る．
    \item 任意の\(F, F' \in \symcal{F}\)について\(F \cap F' \in \symcal{F}\)．
\end{conditions}
\(\{X\} \in \symfrak{A}\)より\(\symfrak{A} \neq \emptyset\)である．
\(\symfrak{Z}\)上の半順序\(\mathord{\preceq}\)を\(\symcal{F} \preceq \symcal{F}' \Leftrightarrow \symcal{F} \subseteq \symcal{F}'\)で定義する．
...
\begin{align*}
    \symcal{E} = \{\tuple{\lambda, F} \mid
       \oxfordcomma{\lambda \in \Lambda}{F \in \symcal{F}_0}{f(\lambda) \in F}
    \}
\end{align*}
\(\symcal{E}\)上の関係\(\mathord{\preceq}\)を\(\tuple{\lambda, F} \preceq \tuple{\lambda', F'} \Leftrightarrow \text{\(\lambda \preceq \lambda'\) and \(F \supseteq F'\)}\)で定義する．
このとき\(\tuple{\symcal{E}, \mathord{\preceq}}\)は有向集合である．
\(\mathord{\preceq}\)が反射律と推移律をみたすことは明らかである．
上界の存在は\(A\)
\end{proof}
...
\begin{proof} \proofimplies{compact-net-compact}{compact-net-univ}普遍ネット\(f\)が\(X\)のどのような元にも収束しないと仮定する．
...
\proofimplies{compact-net-subnet}{compact-net-compact}有限交叉性をもつような\(X\)の閉部分集合族をとり，それを\(\symcal{F}\)と表すことにする．
\(\symcal{E}\)を
