...
\(x\)が\(f\)の集積点なので，\(U_3 := U_1 \cap U_2\)に対して，ある\(\lambda_3 \in \Lambda\)が存在して，
\(\lambda_1 \preceq \lambda_3\), \(\lambda_2 \preceq \lambda_3\), \(x_{\lambda_3} \in U_3\)となる．
ゆえに\(\tuple{\lambda_3, U_3}\)は\(\tuple{\lambda_1, U_3} \preceq \tuple{\lambda_3, U_3}\)かつ\(\tuple{\lambda_2, U_2} \preceq \tuple{\lambda_3, U_3}\)をみたす．

\(\varphi \colon \symcal{D} \to \Lambda\)を\(\varphi\colon \tuple{\lambda, U} \mapsto \lambda\)と定める．このとき\(f \circ \varphi\)は\(f\)のサブネットである．
\(x\)が集積点なので，任意の\(U \in \symcal{U}(x)\)について，任意の\(\alpha \in \Lambda\)に対して，ある\(\beta \in \Lambda\)が存在して，\(x_{\beta} \in U\)が成り立つ．\(\tuple{\beta', U} \succeq \tuple{\beta, U}\)ならば\(\alpha \preceq \varphi(\beta', U) = \beta'\)が成り立つ．
