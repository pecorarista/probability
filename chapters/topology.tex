...
以下の2つは同値である．
\begin{conditions}
    \item\label{compact-equiv-cluster} \(x \in X\)がネット\(f\)の集積点である．
    \item\label{compact-equiv-subnet} \(f\)のサブネットで\(x\)に収束するものが存在する．
\end{conditions}
\end{theorem}
\end{thmbox}

\begin{proof} \(\text{\ref{compact-equiv-cluster}} \Rightarrow \text{\ref{compact-equiv-subnet}}\)：\(\symcal{D} = \{\tuple{\lambda, U} \mid \text{\(\lambda \in \Lambda\), \(U \in \OpenNeighborhood{x}\), and \(f(\lambda) \in U\)}\}\)と定義する．
...
\(\text{\ref{compact-equiv-subnet}} \Rightarrow \text{\ref{compact-equiv-cluster}}\)：任意の\(\kappa \in \Lambda\)と\(U \in \OpenNeighborhood{x}\)をとる．
