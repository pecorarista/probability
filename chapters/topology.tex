...
距離\(d\)が明らかな場合は単に\(\tuple{X, d}\)を\(X\)と書く．
\definitionlabel{distance}
\end{definition}
\end{thmbox}

ここで定義される距離（関数）を指す英語としてmetricのほかにdistanceやdistance functionもよく使われる．
特に以下の例に示すような具体的な距離，ユークリッド距離やマンハッタン距離はそれぞれEuclidean distanceやManhattan distanceのように呼ばれることが多い．
しかし距離空間についてはmetric spaceというのが普通である．

\begin{exa} \(\symbb{R}^n\) \((n \in \PositiveInteger)\)の任意の\(2\)元\(x = (x_1, \ldots, x_n)\)と\(y = (y_1, \ldots, y_n)\)について
\begin{align*}
...
\(\symbb{R}^n\)を距離空間として扱う場合，特に断らない限り，距離はユークリッド距離である．
これは本書に限らず一般的な慣習である．
\end{exa}
...
\begin{tikzpicture}
    ...
    % vertical lines
    \addplot[sLightGray, thick] coordinates {(0.5, -0.25) (0.5, 3.5)};
    \addplot[sLightGray, thick] coordinates {(1.5, -0.25) (1.5, 3.5)};
    \addplot[sLightGray, thick] coordinates {(2, -0.25) (2, 3.5)};
    \addplot[sLightGray, thick] coordinates {(3.5, -0.25) (3.5, 3.5)};

    % horizontal lines
    \addplot[sLightGray, thick] coordinates {(-0.25, 0.5) (4, 0.5)};
    \addplot[sLightGray, thick] coordinates {(-0.25, 1) (4, 1)};
    \addplot[sLightGray, thick] coordinates {(-0.25, 2.25) (4, 2.25)};
    \addplot[sLightGray, thick] coordinates {(-0.25, 2.75) (4, 2.75)};

    \node[circle, draw=black, fill=black, inner sep=0pt, outer sep=0pt, minimum size=0.8mm] (u) at (axis cs:0.5, 0.5) {};
    \node[circle, draw=black, fill=black, inner sep=0pt, outer sep=0pt, minimum size=0.8mm] (v) at (axis cs:3.5, 2.75) {};
    \draw[thick] (u) -- (v);
\end{axis}
\end{tikzpicture}
\end{figure}
...
\begin{exa}
\(X\)を
マンハッタン距離(Manhattan distance)という．
\begin{align*}
    d = \sum_{i = 1}^n \absolute{x_i - y_i}
\end{align*}
\end{exa}
