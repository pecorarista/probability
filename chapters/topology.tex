...
\(2\)次元直交座標上の点\((x_1, y_1)\)と\((x_2, y_2)\)の距離は\(\sqrt{(x_1 - x_2)^2 + (y_1 - y_2)^2}\)で定義された．
...
\begin{definition}
\(X\)を空でない集合とする．
    関数\(d\colon X \times X \to \symbb{R}\)が以下の\ref{distance-nonnegativity}--\ref{distance-triangle-inequality}をみたすとき，\(d\)を\(X\)上の\keyword{距離}(metric)という．
\begin{conditions}
...
\end{align*}
と定義すると\(d\)は距離の公理をみたす（証明略）．この距離を\keyword{ユークリッド距離}(Euclidean distance)という．
...
地球を半径\(R\)の球によってモデル化する．
北極点から測った緯度，すなわち球面座標における天頂角を余緯度(colatitude)といい，
...
\begin{align*}
    ...
    \end{cases}
\end{align*}
経度(longitude)，すなわち球面座標における方位角を\(\lambda\) \((0 \leq \lambda < 2\pi)\)で表す．
\begin{align*}
    \lambda = \begin{cases}
        \displaystyle
    ...
    \end{cases}
\end{align*}
球面上の任意の\(2\)点\(p, q\)をそれぞれ
\begin{align*}
    p = R
    \begin{pmatrix}
        \sin \theta_1 \cos \lambda_1 \\
        \sin \theta_1 \sin \lambda_1 \\
        \cos \theta_1
    \end{pmatrix}, \quad
    q = R
    \begin{pmatrix}
        \sin \theta_2 \cos \lambda_2 \\
        \sin \theta_2 \sin \lambda_2 \\
        \cos \theta_2
    \end{pmatrix}, \quad
\end{align*}
と表す．
先に結論を述べると，\(p\)から\(q\)へ球面に沿って移動する経路で長さが最小となるのは，\(p, q\)そして球の中心\(\symup{O}\)によって定まる平面と，球面が交わってできる円の劣弧の長さ
\begin{align}
    d(p, q) = R \cos^{-1}\left(\frac{p}{\lVert p \rVert} \cdot \frac{q}{\lVert q \rVert}\right) \eqlabel{great-circle-distance}
\end{align}
に等しい．
このような円を\keyword{大円}(great circle)といい，\eqref{great-circle-distance}で定まる距離を\keyword{大円距離}(great circle distance)という（\figref{great-circle-distance}）．
\begin{figure}
    \centering
