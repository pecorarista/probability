...
任意の\(\xi \in \Lambda\)について\(X_\xi\)がコンパクトであることから，\theoremref{compact-net-equiv-props}~\proofimplies{compact-net-compact}{compact-net-univ}より，\(\pi_\xi \circ f\)はある\(x_\xi \in X_\xi\)に収束する．
\end{proof}
...
\begin{proof}\proofimplies{net-whole-convergence}{net-elementwise-convergence}：\(\projection{\xi} \circ f\)連続性により\(\projection{\xi} (x) = x_\xi\)

\proofimplies{net-elementwise-convergence}{net-whole-convergence}：b
...
\begin{proof}
\proofimplies{net-continuous}{net-f-lim-x-lim-f-x}：\(f\)が\(x_0 \in X\)で連続かつ\(g \to x_0\)ならば，\((f \circ g) \to f (x_0)\)を示す．
\(f(x_0)\)の任意の近傍\(V\)をとると，連続性により\(f^{-1}(V)\)は\(x_0\)の開近傍である．
\(g \to x_0\)より，ある\(\lambda_0\)が存在して\(\lambda \succeq \lambda_0\)ならば\(g(\lambda) \in f^{-1}(V)\)，すなわち\(f(g(\lambda)) \in f(f^{-1}(V)) \subseteq V\)が成り立つ．

\proofimplies{net-f-lim-x-lim-f-x}{net-continuous}：\(x_0 \in X\)において\(f\)が連続でないと仮定する．
...
ネット\(g\)を\(g \colon U \mapsto x_U\)で定義すると，\(g\)は\(x_0\)に収束する．
したがって\(f \circ g\)が\(f(x)\)に収束しない．
\end{proof}
