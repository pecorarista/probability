...
\noindent 混乱のおそれがない場合は\(0_A\)や\(1_A\)を\(0\)や\(1\)と表す．
...
\(X\)の任意の異なる2点\(x_1, x_2\)に対して，\(f(x_1) \neq f(x_2)\)となるような\(f \in S\)が存在するとき，\(S\)は\(X\)を\keyword{分離}するという．
\eqref{subalgebra-polynomial}で定義された\(S\)は，\(\identity\colon x \mapsto x\)を元としてもつため，\(\identity(x_1) \neq \identity(x_2)\)となり\(X\)を分離する．一方で\eqref{subalgebra-polynomial-even}の\(S'\)は，どのような\(f\in S'\)をとっても\(f(-1) =  f(1)\)となり\(X\)を分離することができない．
...
\lemmaref{unital-subalgebra-separation}

\begin{thmbox}
\begin{lemma}
\(C(X, \symbb{K})\)の単位的な部分代数\(S\)が\(X\)を分離するならば，
\(X\)の任意の異なる2点\(x_1, x_2\)と任意の定数\(\alpha, \beta \in \symbb{K}\)について
\begin{align*}
  h(x_1) = \alpha,\quad h(x_2) = \beta
\end{align*}
となるような\(h \in S\)が存在する．\lemmalabel{unital-subalgebra-separation}
\end{lemma}
\end{thmbox}

\begin{proof}
\(S\)が\(X\)を分離することから，\(X\)の異なる任意の2点\(x_1, x_2\)に対して，\(f(x_1) \neq f(x_2)\)となるような\(f \in S\)が存在する．このとき\(h\)を
\begin{align*}
    h(x) = \alpha 1_{C(X)}(x) + (\beta - \alpha) \frac{f(x) - f(x_1)}{f(x_2) - f(x_1)}
\end{align*}
と定義すると，これは条件を満たす．
\end{proof}
...
まず\(\lvert f \rvert\)が \(a_1 f + a_2 f^2 + \cdots + a_n f^n \in S\)で近似できることを示す\footnote{%
\(S\)が単位的であると仮定していないため，定数関数\(a_0 1_{C(X)}(x)\)が\(S\)の元とは限らないことに注意する．}．
...
\begin{gather*}
  p'(t) = \frac{1}{2} (1 - t)^{-1/2} \cdot (-1) = -\frac{1}{2}(1 - t)^{-1/2}, \\
  p''(t) = -\frac{1}{2}\cdot \left(-\frac{1}{2}\right) \cdot (1 - t)^{-3/2} \cdot (-1) = - \frac{1}{4}(1 - t)^{-3/2}, \\
  p'''(t) = -\frac{1}{4} \cdot \left(-\frac{3}{2}\right) \cdot (1 - t)^{-5/2} \cdot (-1) = - \frac{3}{8} (1 - t)^{-5/2}, \\
  \vdots \\
  p^{(n)}(t) = - \frac{(2 n - 3) !! }{2^n} (1 - t)^{- (2 n - 1)/2} \ (n \geq 2).
\end{gather*}
...
\end{align}
となるため，ratio testではなくRaabeの判定法を試みる：
\begin{align*}
b_n
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \addplot[sApproxCurve, thick, dotted][domain=-1.2:1.17] {%
            1
            - x/2
            - x^2 / 8
            - 3 * x^3 / 16
...
\begin{figure}
    ...
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[sSimpleCurve, thick, dashed][domain=-1.6:1.77] {1 - (1 - x^2)/2};
            \addplot[sApproxCurve, thick, dotted][domain=-1.7:1.7] {%
                1
                - (1 - x^2)/2
                - (1 - x^2)^2 / 8
                - 3 * (1 - x^2)^3 / 16
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{関数\(t \mapsto |t|\)の多項式による近似．}\figlabel{stone-weierstrass-approx-abs}
\end{figure}
...
\(t \mapsto |t|\)を多項式近似することができたので，これで\(|f|\)の多項式近似を考えることができる．\(\|f\| = 0\)，すなわち\(f = 0_{C(X)}\)のときは\(|f| = f\in S\)となる．よって以下では\(\|f\| > 0\)とする．このとき
\begin{align*}
  |f| = \|f\| \frac{|f|}{\|f\|}
\end{align*}
